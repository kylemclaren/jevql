...
export type HookSidebarProps = Omit<ComponentProps<"nav">, "onChange"> & {
  ...
  color?: string;
  dashed?: boolean;
  /** current location; items whose href matches are active */
  pathname?: string;
};
...
export function HookSidebar({
  ...
  const routeIndex = items.findIndex((item) => norm(hrefOf(item)) === norm(pathname));
    ...
    >
      {label && (
        <span
          ...
          className="pb-3 pl-0.5 pr-2 text-[0.66rem] font-black uppercase tracking-[0.08em] text-ink"
        >
          {label}
        </span>
      )}
      ...
      >
        ...
        {items.map((item, index) => {
          ...
          const rowProps = {
            ...
            className: cn(
              "py-1.5 pl-5 pr-2 text-left text-[0.84rem] font-bold leading-snug transition-colors duration-200 motion-reduce:transition-none",
              isActive
                ? "text-ink"
